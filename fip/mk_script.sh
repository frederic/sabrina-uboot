...
function build_blx_src() {
	...
	if [ $name == ${BLX_NAME[0]} ]; then
		...
		cp $bin_folder/aml_ddr.fw  $FIP_FOLDER/$soc
...
function update_src_folder() {
	dbg "Update BLX_SRC_FOLDER[$1]=$2"
	BLX_SRC_FOLDER[$1]=$2
}
...
function build() {
	...
	if [ ! $CONFIG_BOARD_NAME ]; then
		CONFIG_BOARD_NAME=null
	fi
	build_uboot ${CONFIG_BOARD_NAME} ${CONFIG_SYSTEM_AS_ROOT} ${CONFIG_AVB2}
	...
	while [ $i -lt $# ]; do
		...
		case "$arg" in
			...
			--update-bl32)
				...
				continue ;;
			--update-bl2-src)
				update_src_folder 0 "${argv[@]:$((i))}"
				continue ;;
			--update-bl30-src)
				update_src_folder 1 "${argv[@]:$((i))}"
				continue ;;
			--update-bl31-src)
				update_src_folder 2 "${argv[@]:$((i))}"
				continue ;;
			--update-bl32-src)
				update_src_folder 3 "${argv[@]:$((i))}"
				continue ;;
			--ddrfw)
				CONFIG_DDR_FW=1
				export CONFIG_DDR_FW
				continue ;;
			--board_name)
				CONFIG_BOARD_NAME="${argv[$i]}"
				echo "export CONFIG_BOARD_NAME: ${CONFIG_BOARD_NAME}"
				export CONFIG_BOARD_NAME="${argv[$i]}"
				continue ;;
			--systemroot)
